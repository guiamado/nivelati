<template>
  <v-app-bar app color="grey lighten-4">
    ...
    <v-btn text @click="mudarRota('/relatorio')" v-if="token">
      <span :class="{ 'blue--text': isTelaRelatorio }">Relatórios</span>
    </v-btn>
...
export default {
  ...
  methods: {
    ...
    verificarRotas() {
      ...
      if (this.$route.path === "/relatorio") {
        this.isTelaRelatorio = true;
      }
    },
    mudarRota(rota) {
